<?php
	...
	if($transacao->transacao->erro){
		...
		echo "<div class='row-fluid'>".
			"<div class=span12>".
			"<p>Houve um erro ao processar o pagamento, a seguinte mensagem foi
			retornada pela operadora de cartão de crédito.</p>".
			heading("Código do erro: ". $transacao->transacao->erro->codigo, 4) .
			heading("Mensagem da operadora: ".
			$transacao->transacao->erro->mensagem, 4) .
			"</div>" . "</div>";
	}else{
		...
		echo "<div class='row-fluid'>".
			...
			"<p>Seu pagamento foi processado pela administradora de cartão de 
			crédito com o seguinte status.</p>".
