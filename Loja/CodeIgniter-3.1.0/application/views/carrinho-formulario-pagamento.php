<?php
	...
	echo form_open(base_url("carrinho/finalizar_compra")) .
		...
		"</div>" . "<div class='span1'> </div>" . "<div id='dados_cartao'>" . '<div class=span4 >';
	...
	$parcelas = array(
		=>'1 parcela de ' . reais($this->cart->total() + $frete),
		=>'2 parcelas de ' . reais(($this->cart->total() + $frete)/2),
		=>'3 parcelas de ' . reais(($this->cart->total() + $frete)/3));
	echo form_label('Parcelamento','parcelamento') .
		form_dropdown('parcelamento', $parcelas) .
		'</div> </div>'.
		form_submit(array ('id'=>'pagar', 'value'=>'Pagar e finalizar compra', 'class'=>'a-direita') ) . form_close() .
		"</div>";
?>
